...
import type { ExecutionStep, AssertType } from '../../storage/schemas.js';
import { getPageSnapshotFromPage } from '../../browser/playwright-adapter.js';
...
export async function generatePostStepAssertion(
  ...
  const snapshot = await getPageSnapshotFromPage(page).catch(() => null);
